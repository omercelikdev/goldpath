...
| [goldpath-approvals](goldpath-approvals.md) | Goldpath.Approvals — human approval workflows: maker-checker, four-eyes, amount-laddered authority, delegation, escalation, worklist; definitions as versioned data | **proposed** — the T21 shelf prepared; build waits on the Ring B trigger (§5.1 by-product rule) |
| [goldpath-fileexchange](goldpath-fileexchange.md) | Goldpath.FileExchange — file-based integration rails as a unit: scheduled pick-up/drop, format contracts, idempotent `(file,line)` ingestion, quarantine, reprocessing, archival, outbound | **proposed** — the T22 shelf prepared; build waits on the Ring B trigger (§5.1 by-product rule) |
