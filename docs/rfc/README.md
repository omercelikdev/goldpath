...
| [goldpath-messaging-exit](goldpath-messaging-exit.md) | the messaging dependency: measured exposure, options, and the triggers that would move us | **draft** — owner decision pending (recommendation: stay pinned on the 8.x line; a move would be a MAJOR version, not an internal swap) |
