...
| U3 | Module panels (bulk/campaign/notification/archival), capability-lit | each panel drives its module's real verbs in smoke — **bulk MET** (2026-07-27): the smoke host composes bulk, and Playwright uploads a real file, reads the report the engine wrote, is refused by the engine when invalid rows block approval, rejects with the mandatory reason, then approves a clean file; **campaign MET** (2026-07-27): the smoke host composes campaign with a REAL broker, and Playwright watches the pacer release, throttles it (only the changed field travels), pauses, resumes and aborts it with the mandatory reason — reading the verb log back; **notification MET** (2026-07-27): the smoke host composes notification with a real webhook, a dead SMTP transport and a MaySend refusal, and Playwright reads all three kinds of evidence back — sent, failed with the transport's own words, suppressed with its reason — proving the recipients stay masked and the surface offers no verb; archival remains open |
