...
- [x] B: both worker triggers generate, build, smoke; manifest kind validates (lean by
  ...
  **Still open from that decision:** `goldpath add worker` (Slice C's in-solution worker)
  writes its OWN embedded skeleton with the jobs admin surface hard-coded to
  `exposeUnsecured: true`; it must learn `--auth` and `--features` the same way, or the
  in-solution worker stays behind the standalone one. Its own PR; the proof is the
  finance-sample shape (API + consumer + EOD worker) generated by the verb with the floor up.
