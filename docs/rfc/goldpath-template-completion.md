...
- [x] B: both worker triggers generate, build, smoke; manifest kind validates (lean by
  ...
  **2026-09-03 addendum — concept parity (open-threads T25):** "lean by schema design" was
  ...
  `GmWorkerQueueFeatures` — both GREEN on the 2026-09-03 dispatch (T25 closed).
  **2026-09-05 — the in-solution worker caught up, by INHERITANCE rather than flags:**
  `goldpath add worker` reads the solution's composition root and gives the new worker the
  same floor (`AddGoldpathAuth` + `UseGoldpathAuth`, the API-key strategy when that is the
  solution's; the fleet's admin surface behind the SAME ops policy instead of the hard-coded
  opt-out) and the same cross-cutting features on its own context (audit rows, soft-delete
