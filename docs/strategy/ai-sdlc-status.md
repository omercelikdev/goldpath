...
| Development | `new-service` / `add-feature`: spec → code with tests (§7) | `goldpath new`/`add worker` deterministic scaffolding **SHIPPED-PROVEN** (GM nightly); `goldpath-feature` skill **SHIPPED-PROVEN** — it drove the five CorPay S2 slices (`coverage-matrix.md` tooling table); the `goldpath add feature` CLI verb is **SHIPPED-PROVEN** by the nightly `GmGrown` shape (a lean app grown by five recipes — audittrail, softdelete, locking, bulk, outbox — then migrated, built, drift-checked and smoked; GREEN on the 2026-09-05 dispatch after the four recipe bugs it found were fixed); the Insurance sample remains its field proof | build + analyzers (42 rules) + PublicAPI ledger + `dotnet format` | field the CLI verb (coverage-matrix plans it for the Insurance sample) |
...
| Operations | admin APIs + ops packs + console (§7.1) | Admin APIs + dashboards + runbooks **SHIPPED-PROVEN** per module (every shipped module carries `ops/` since 2026-08-03 — the Idempotency/AuditTrail/SoftDelete runbooks closed the last three gaps, honest-signals posture: no unshipped metric is cited); console **SHIPPED-PROVEN** (U1–U9 met 2026-08-03: cross-service registry, TODAY triage with stat cards, the full family standards ui-standard v1.3 §7–§9, contract R3 OR filters, `MapGoldpathConsole()` with canonical-root redirect — 39 smoke tests against three real apps — the journey, served-console, screenshot and axe specs, counted by the run itself (2026-09-05: the approval worklist journey and the Approvals/File rails axe scans joined; the panel count the axe spec walks is eight) and the app-SERVED console; the OIDC login remains ledgered as T10; **T1 is still OPEN**: CorPay's nightly asserts that the console is SERVED and inherits the floor (401 on the secured head, 200 on the internal head's visible opt-out) — it does not DRIVE it. This row claimed the proof was riding since 2026-09-03; the coverage audit of 2026-09-05 found the two ledgers disagreeing and this is the one that was wrong) | admin contract frozen + integration proofs | console smoke 39 tests + axe |
...
| Skills | active recipes | PARTIAL — three fielded, one not (see §2) |
