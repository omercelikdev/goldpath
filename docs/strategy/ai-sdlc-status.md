...
| Development | `new-service` / `add-feature`: spec → code with tests (§7) | `goldpath new`/`add worker` deterministic scaffolding **SHIPPED-PROVEN** (GM nightly); `goldpath-feature` skill **SHIPPED-PROVEN** — it drove the five CorPay S2 slices (`coverage-matrix.md` tooling table); the `goldpath add feature` CLI verb rides the nightly `GmGrown` shape since 2026-09-04 (a lean app grown by five recipes — audittrail, softdelete, locking, bulk, outbox — then built, drift-checked and smoked): **SHIPPED-PROVEN** the night that shape is green; the Insurance sample remains its field proof | build + analyzers (42 rules) + PublicAPI ledger + `dotnet format` | field the CLI verb (coverage-matrix plans it for the Insurance sample) |
