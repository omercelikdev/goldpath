...
| GmWorkerInSolution | `--features audittrail,softdelete,multitenancy` (auth openid) + `add worker eod --trigger jobs;add worker ingest --trigger queue` | the in-solution worker inherits the solution's auth floor and features (T25 parity by inheritance): both workers build, boot behind the AppHost and pass drift (with the SPEC0203 WARN on the jobs worker's Goldpath.Jobs — open-threads T26; the lane fails on ERROR only) |
