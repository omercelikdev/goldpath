...
| GmCleanFeatures | `--broker none --layout clean-architecture --features idempotency --features dataprotection --features locking` | the clean layout carrying the features whose CODE THE LAYOUT MOVES. Neither clean shape above reaches them: GmFourClean takes no features, and GmOneClean keeps the default broker, which supplies `Mediant.Behaviors` to Application by accident through `Goldpath.Messaging` — hence broker-less here. Added 2026-09-09 after qorpe/coexist generated this shape as its first adopter and it did not build, twice: the entity partials and the command attribute land in Domain and Application, but only the Api csproj carried the feature-conditional package references. `--features audittrail` joins the row at the train that publishes the `IAuditLogged` move (release-checklist next-train adoptions) |
