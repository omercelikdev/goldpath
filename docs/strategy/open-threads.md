...
| T13 | Frozen JOBS routes with **no screen**: `pause-all`/`resume-all`, `reschedule`, the calendar CRUD, the global `/audit` | Found 2026-07-28 by reading the frozen inventory against the console's client. `pause-all` is the one that matters — the single verb an operator reaches for during an incident, and the console cannot reach it. Nothing was deferred here on purpose; it was simply never noticed, which is why it is written down now | U5 (with the R2 routes — the screens land together) | the console smoke drives every one of them against a real fleet: a paused fleet stops firing and resumes, a rescheduled job fires on its new cron, a calendar excludes a day, and the audit tab shows who did it |
