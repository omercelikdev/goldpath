...
| T15 | **Campaign R1 — device-fleet parity** (`ExcludedDays`, `EndDate`, per-item auto-retry ladder, `GlobalTps`) | A device-management-class plan needs six dials; the policy carries four. RFC revision written (goldpath-campaign.md R1, PROPOSED) — additive, defaults = today's behavior | owner approves the R1 revision → next train | the R1 test plan runs: excluded-day flip and end-date expiry driven in the console smoke against a real campaign; two campaigns under one GlobalTps never jointly exceed it |
