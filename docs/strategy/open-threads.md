...
| T14 | Four admin responses were **untyped in OpenAPI** ([#98](https://github.com/omercelikdev/goldpath/issues/98)) — `.Produces<T>` LANDED 2026-08-03 on all four (archival holds/erasures, bulk batches/errors) plus a RESPONSE-side export test (the exact hole that let #98 ship a full train) | The remaining half is the consumer proof: CorPay builds against the PUBLISHED packages, so its re-export regains the schemas only when preview.6 carries them | the preview.6 train (CorPay pin bump) | CorPay's re-export regains the four schemas and `goldpath check` stays green |
