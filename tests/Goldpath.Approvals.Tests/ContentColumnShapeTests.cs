...
public sealed class ContentColumnShapeTests
{
    private sealed class ConventionsContext(DbContextOptions<ConventionsContext> options) : DbContext(options)
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder) => modelBuilder.AddGoldpathApprovalModel();
    }
    ...
    public void Document_columns_declare_their_own_shape()
    {
        ...
        Assert.Equal(-1, context.Model.FindEntityType(typeof(GoldpathApprovalRequest))!.FindProperty("Trail")!.GetMaxLength());
        Assert.Equal(1024, context.Model.FindEntityType(typeof(GoldpathApprovalRequest))!.FindProperty("Reason")!.GetMaxLength());
    }
}
