...
public sealed class ContentColumnShapeTests
{
    private sealed class ConventionsContext(DbContextOptions<ConventionsContext> options) : DbContext(options)
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder) => modelBuilder.AddGoldpathCampaign();
    }
    ...
    public void Document_columns_declare_their_own_shape()
    {
        ...
        Assert.Equal(-1, context.Model.FindEntityType(typeof(GoldpathCampaign))!.FindProperty("ParametersJson")!.GetMaxLength());
        Assert.Equal(-1, context.Model.FindEntityType(typeof(GoldpathCampaignItem))!.FindProperty("TargetJson")!.GetMaxLength());
    }
}
