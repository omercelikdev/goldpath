...
public sealed class ContentColumnShapeTests
{
    private sealed class ConventionsContext(DbContextOptions<ConventionsContext> options) : DbContext(options)
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder) => modelBuilder.AddGoldpathNotification();
    }
    ...
    public void Document_columns_declare_their_own_shape()
    {
        ...
        Assert.Equal(-1, context.Model.FindEntityType(typeof(GoldpathNotification))!.FindProperty("Body")!.GetMaxLength());
    }
}
